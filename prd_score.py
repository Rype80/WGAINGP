...
def plot(precision_recall_pairs,plotcols, Impute_Cols, LabeL=None, out_path=None,
         legend_loc='lower left', dpi=300, test=False, name="Gain"):
  ...
  if LabeL is not None and len(LabeL) != len(precision_recall_pairs):
    raise ValueError(
        'Length of labels %d must be identical to length of '
        'precision_recall_pairs %d.'
        % (len(LabeL), len(precision_recall_pairs)))

  N_cols= len(precision_recall_pairs)
  plotrows= (int) (np.ceil(N_cols/plotcols))
    
  # Colors 
  colors=cycle(["brown", "brown" , "forestgreen", "forestgreen",  "teal", "teal", 
                "maroon", "maroon", "navy","navy", "olive", "olive"])  
   
  if(len(Impute_Cols)==2):
      title_size=11
      label_size=14
      f,a = plt.subplots(2,1, sharex=True, sharey=True, figsize=[5,5])
      f.suptitle("Precision vs Recall Distribution Plot", size=title_size, y=0.95)
      f.text(0.5, 0, 'Recall', ha='center', size=label_size)
      f.text(0.18, 0.50, 'Precision', va='center', rotation='vertical', size=label_size)
      a = a.ravel()
      i=0
      for idx,ax in enumerate(a):
        if(i<len(LabeL)):
              precision, recall = precision_recall_pairs[i]
              ax.plot(recall, precision, alpha=0.5, linewidth=3, color=next(colors))
              ax.fill_between(recall, precision, color=next(colors))
              plt.xlim([0, 1])
              plt.ylim([0, 1])
              ax.set(adjustable='box', aspect='equal')
              ax.set_title(LabeL[i], size=title_size)
              i+=1
      plt.tight_layout()
      if out_path is None:
       plt.show()
  ...
  if(len(Impute_Cols)==1):
      ...
      f.suptitle("Precision vs Recall Distribution Plot", size=title_size, y=0.95)
      f.text(0.5, 0, 'Recall', ha='center', size=label_size)
      f.text(0, 0.50, 'Precision', va='center', rotation='vertical', size=label_size)
      i=0
      precision, recall = precision_recall_pairs[i]
      ax.plot(recall, precision, alpha=0.5, linewidth=3, color=next(colors))
      ax.fill_between(recall, precision, color=next(colors))
      ...
      ax.set(adjustable='box', aspect='equal')
      ax.set_title(LabeL[i], size=title_size)
      plt.tight_layout(h_pad=12, w_pad=0.05)
      i+=1
      if out_path is None:
       plt.show()
      else:
